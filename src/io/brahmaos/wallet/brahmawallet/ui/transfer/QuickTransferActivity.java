...
public class QuickTransferActivity extends BaseActivity {
        ...
        btcTxBroadcastComplete.onBackpressureBuffer()
                ...
                .subscribe(new Observer<String>() {
                    ...
                    public void onNext(String txHash) {
                        if (txHash != null && txHash.length() > 0 && tvTransferStatus != null && customStatusView != null) {
                            ...
                            new Handler().postDelayed(() -> {
                                ...
                                intent.putExtra(IntentParam.PARAM_PAY_HASH, txHash);
    ...
    private void initView() {
        mScrollViewContent = findViewById(R.id.scroll_view_content);
        mScrollViewContent.setVisibility(View.VISIBLE);
        mScrollViewContent.setAnimation(AnimationUtil.makeTopAnimation());
    ...
    private void showChosenAccountInfo(AccountEntity account) {
        ...
        if (account.getType() == BrahmaConst.BTC_ACCOUNT_TYPE) {
            accountBalance = new BigInteger(String.valueOf(BtcAccountManager.getInstance().getBtcAccountBalance(account.getAddress())));
            String accountBalanceStr = String.valueOf(CommonUtil.convertUnit(BrahmaConst.BITCOIN, accountBalance));
    ...
    private void getEthereumChainBalance(List<AccountEntity> accounts, String tokenAddress) {
        for (AccountEntity account : accounts) {
            EthAccountManager.getInstance().getEthereumBalanceByAddress(account, chosenToken)
    ...
    private void showEthPasswordDialog() {
        ...
        AlertDialog passwordDialog = new AlertDialog.Builder(QuickTransferActivity.this, R.style.Theme_AppCompat_Light_Dialog_Alert_Self)
                ...
                .setPositiveButton(R.string.ok, (dialog, which) -> {
                    ...
                    EthAccountManager.getInstance().sendTransfer(chosenAccount, chosenToken, password, receiptAddress,
        ...
        AlertDialog passwordDialog = new AlertDialog.Builder(QuickTransferActivity.this, R.style.Theme_AppCompat_Light_Dialog_Alert_Self)
                ...
                .setPositiveButton(R.string.ok, (dialog, which) -> {
                    ...
                    if (isBtcValidPassword(password)) {
                        // show transfer progress
                        mLayoutTransferStatus.setVisibility(View.VISIBLE);
                        String txHash = BtcAccountManager.getInstance().transfer(chosenAccount.getAddress(),
                                receiptAddress, password, CommonUtil.convertBTCFromSatoshi(transferValue), feePrice, "");
                        if (txHash != null && txHash.length() > 0) {
                            customStatusView.loadLoading();
                        } else {
                            customStatusView.loadFailure();
